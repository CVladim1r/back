...
async def find_room(sid: str):
    ...
    if room_id:
        # Handle WebSocket part separately
...
async def websocket_endpoint(websocket: WebSocket):
    ...
    try:
        await websocket.send_json({'type': 'connected', 'message': 'Welcome to Durak Online!'})
        while True:
            data = await websocket.receive_json()
            if data['type'] == 'inviteFriend':
                await on_invite_friend(websocket, data)
            else:
...
async def play_card(websocket: WebSocket, sid, data):
